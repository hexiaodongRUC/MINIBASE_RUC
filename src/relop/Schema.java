...
public class Schema {
	...
	public Schema(int fldcnt) {
  	...
  	/**
  	 * Constructs a schema from another schema with specific fields
  	 * @param schema source schema
  	 * @param fields fields to reserve
  	 */
  	public Schema(Schema schema, Integer... fields) {
	  	types = new int[fields.length];
	    lengths = new int[fields.length];
	    offsets = new int[fields.length];
	    names = new String[fields.length];
	    for(int i=0; i<fields.length; i++) {
	    	this.initField(i, schema, fields[i]);
	    }
  	}
  	...
  	public void print() {
  		...
  		for (int i = 0; i < types.length; i++) {
  			...
  			for (int j = 0; j < collen - names[i].length() - 1; j++) {
  				System.out.print(" ");
  			}
